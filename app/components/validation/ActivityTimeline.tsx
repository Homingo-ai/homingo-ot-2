            ...
            <div className="observation-card flex-1 bg-white rounded-xl border border-border p-4 shadow-sm max-w-full">
                <div className="mb-3">
                    <div className="flex justify-between items-start mb-1 flex-wrap gap-2">
                        <div>
                            <div className="text-sm font-extrabold text-text-main">{obs.authorName || 'Unknown'}</div>
                            <div className="text-xs text-text-dim font-semibold">{obs.authorRole || 'OT'}</div>
                        </div>
                    ...
                    <div className="flex gap-2 flex-wrap mt-2">
                        ...
                        >
                            ...
                            <span className="text-[11px] font-extrabold" style={{ color }}>{obs.category}</span>
                        </div>

                        <div className={cn(
                            "inline-flex items-center gap-1.5 py-1 px-2.5 rounded-md border",
                            obs.includeInReport ? "bg-emerald-50 border-emerald-500/25" : "bg-slate-50 border-border"
                        )}>
                            <div className={cn(
                ...
                <p className="observation-content text-[13px] leading-relaxed text-text-main m-0 break-words">
                    {displayContent}
                </p>
...
const ActivityTimeline: React.FC<ActivityTimelineProps> = ({ observations, onAddFollowup }) => {
    ...
    return (
        <div className="relative">
            <div
                className="timeline-line absolute left-5 top-8 bottom-8 w-0.5 bg-gradient-to-b from-slate-200 to-transparent"
            />
